...
window.onload = function() {
  ...
  this.upButton.style.top = this.winHeight - gap + "px";
  ...
  var req = createCORSRequest("GET", "content/content.html");
  root.req = req;
  root.req.onreadystatechange = function(ev) {
...
function activateNode(current = new NavNode(), addSwiper = true) {
  ...
  a.onMove = function(sw) {
    ...
    if (selected.childControlDiv.a) {
      var pos = -Math.round(
        selected.childControlDiv.a.currentX / thumbnailWidth
      );
      lastEl = [];
      for (var i = pos - 2; i < pos + 3; i++) {
        if (isValidIndex(i, selected.children)) {
          ...
          var d = Math.abs(
            i * thumbnailWidth + selected.childControlDiv.a.currentX
          );
...
function readDocument() {
  ...
  while (document.body.firstElementChild) {
    if (
      document.body.firstElementChild.nodeName.charAt(0) == "H" &&
      document.body.firstElementChild.nodeName.length == 2
    ) {
      var parent = root;
      var index =
        parseInt(document.body.firstElementChild.nodeName.charAt(1)) - 1;
